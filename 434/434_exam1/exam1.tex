...
\setmainfont[Ligatures={TeX, Common}, BoldFont={* Bold}, ItalicFont={* Italic}, Fractions ={On}, Numbers={Proportional, OldStyle}]{\mainfont}
...
\AnswerBox{0.1\textheight}{No. Although larvae may have the potential to disperse far, they may not actually do so. Several studies have shown that some species with high potential do not disperse.}

\question[5]
I listed five requirements for ecosystem-based management. List and briefly explain any two of them.

\begin{AnswerPage}{0.25\textheight}%
	The five requirements are \\%
	1) goals encompass all ecosystem services, \\% 
	2) the spatial scale is based on natural boundaries across multiple ecosystems, \\%
	3) all sectors of human use are integrated, \\%
	4) cumulative effects across sectors are estimated, and \\%
	5) strategies adapt over time to account for uncertainty.

	\medskip

	2.5 points each explanation.
\end{AnswerPage}

\question[10]
Explain the difference between ecosystem function and ecosystem services. Explain how increased ecosystem function affects ecosystem services. Provide an example of your choosing to show this relationship.
...
\question[5]
Explain how the effects of warming temperatures on larval growth might affect connectivity and retention among marine protected areas in the Mediterranean.

	\AnswerBox{0.15\textheight}{%
	\emph{4 points.} Warming temps cause larvae to grow faster due to higher metabolism. Thus, larve mature faster so might not disperse as far. Shorter dispersal times / distances should then reduce connectivity and increase retention.
}

\question[15]
Name the three non-population-based approaches to ecosystem restoration and explain the difference in emphasis between them. Explain why that emphasis is important for \emph{each} approach.
...
\question[10]
Compare and contrast the insurance hypothesis and the portfolio effect.  Explain how these confer stability under the high redundancy model of ecosystem function.
